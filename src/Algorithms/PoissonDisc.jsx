...
export const generatePoissonDiscPoints = (mapWidth, mapHeight, r, k) => {
  ...
  // start point
  if (points.length === 0) {
    addPoint(points, activePoints, grid, mapWidth, mapHeight, cellSize, [getRandom(0, mapWidth), getRandom(0, mapHeight)]);
  }
  ...
  for (var i = 0; i < 100; i++) {
    ...
    for (let i = 0; i < k; i++) {
      ...
      // accept valid first candidate and break
      if (x >= 0 && y >= 0 && x < mapWidth && y < mapHeight && !isWithinRangeOfSamples(grid, gridWidth, gridHeight, cellSize, [x, y], r)) {
        addPoint(points, activePoints, grid, gridWidth, gridHeight, cellSize, [x, y]);
...
const addPoint = (pointList, activePointList, grid, gridWidth, gridHeight, cellSize, point) => {
  pointList.push(point);
  activePointList.push(point);
  const gridId = gridWidth * Math.floor(cellSize / point[0]) + gridHeight * Math.floor(cellSize / point[1]);
  grid[gridId] = point;
};

const isWithinRangeOfSamples = (grid, gridWidth, gridHeight, cellSize, point, r) => {
  ...
  const maxCell = [Math.min(gridWidth, gridCell[0] + 3), Math.min(gridHeight, gridCell[1] + 3)];
